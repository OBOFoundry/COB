...
1. Add an import statement to your `src/ontology/cob-edit.owl` file. We suggest to do this using a text editor, by simply copying an existing import declaration and renaming it to the new ontology import, for example as follows:
    ```
    ...
    Ontology(<http://purl.obolibrary.org/obo/cob.owl>
    Import(<http://purl.obolibrary.org/obo/cob/imports/ro_import.owl>)
...
1. remove the import declaration from your `src/ontology/cob-edit.owl`.
2. remove the id from your `src/ontology/cob-odk.yaml`, eg. `- id: go` from the list of `products` in the `import_group`.
...
Now add a new goal in your custom Makefile (`src/ontology/cob.Makefile`, _not_ `src/ontology/Makefile`).
...
1. Open `src/ontology/cob-odk.yaml`
...
1. Providing a `components/mycomp.owl:` make target in `src/ontology/cob.Makefile`
...
1. Providing an additional attribute for the component in `src/ontology/cob-odk.yaml`, `source`,
